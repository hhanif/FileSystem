...
class FileSystem {
  ...
  // Helper method to resolve a path to an item in the filesystem
  resolvePath(path) {
  ...
  // Helper method to resolve a directory path
  ...
  cp(sourcePath, destinationPath, recursive = false) {
    const source = this.resolvePath(sourcePath);
    const destinationDirectory = this.resolveDirectory(destinationPath);

    if (source && destinationDirectory) {
      const sourceName = sourcePath.split("/").pop();
      const destinationPathExists = destinationDirectory.children[sourceName];

      if (
        destinationPathExists &&
        destinationDirectory.children[sourceName].type === "directory"
      ) {
        this.mergeDirectories(
          source,
          destinationDirectory.children[sourceName],
          recursive,
        );
      } else {
        const uniqueName = this.getUniqueName(
          sourceName,
          destinationDirectory.children,
        );
        destinationDirectory.children[uniqueName] =
          this.cloneFileOrDirectory(source);
      }
  ...
  createReadStream(path, options = {}) {
    ...
    if (file && file.type === "file") {
      ...
      return {
        read: (size) => {
          const chunk = file.content.slice(position, position + size);
  ...
  createWriteStream(path, options = {}) {
    ...
    if (file && file.type === "file") {
      ...
      return {
        write: (data) => {
          file.content =
            file.content.slice(0, position) +
            data +
            file.content.slice(position + data.length);
  ...
  walk(path, callback) {
    ...
    if (directory && directory.type === "directory") {
      this.walkRecursive(directory, callback);
    }
  }

  walkRecursive(directory, callback) {
    for (const child of Object.values(directory.children)) {
      if (child.type === "directory") {
        if (callback(child) !== false) {
          this.walkRecursive(child, callback);
        }
      } else {
        callback(child);
      }
  ...
  mergeDirectories(source, destination, recursive) {
    for (const [name, sourceChild] of Object.entries(source.children)) {
      const destinationChild = destination.children[name];

      if (
        destinationChild &&
        destinationChild.type === "directory" &&
        recursive
      ) {
        this.mergeDirectories(sourceChild, destinationChild, true);
      } else {
        const uniqueName = this.getUniqueName(name, destination.children);
        destination.children[uniqueName] =
          this.cloneFileOrDirectory(sourceChild);
      }
    }
  }

  getUniqueName(name, existingChildren) {
    let uniqueName = name;
    let count = 1;

    while (existingChildren[uniqueName]) {
  ...
  processCommand(command) {
    ...
    switch (cmd) {
      ...
      case "chown":
        ...
        break;
      case "cp":
        this.cp(args[1], args[2], args[3] === "-r");
        break;
      case "read":
        const readStream = this.createReadStream(args[1], {
          start: parseInt(args[2]),
        });
        console.log(readStream ? readStream.read(parseInt(args[3])) : null);
        break;
      case "write":
        const writeStream = this.createWriteStream(args[1], {
          start: parseInt(args[2]),
        });
        writeStream && writeStream.write(args.slice(3).join(" "));
        break;
      case "walk":
        this.walk(args[1], (item) => console.log(item.name));
        break;
      case "exit":
        this.rl.close();
        break;
